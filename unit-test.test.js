...
describe("Unit test challenge!", function () {
  ...
  it( "Task 6-A - Comparison operator - less than true", function() {
    assert.equal(compOp(23, 24), true );
    });

  it(" Task 6-B - Comparison operator - less than false", function() {
    assert.equal(compOp(25, 24), false );
    });
  
  it( "Task 7-A - Logical operator - && false", function() {
    assert.equal(logicOp(12, 30, 11, 11), true );
    });

  it( "Task 7-B - Logical operator - && false", function() {
    assert.equal(logicOp(12, 30, 10, 11), false );
    });
  it( "Task 8 - Change string var to a number", function() {
    assert.equal(typeof(changeStrToNumber("Five")), "number");
  });
  it("Task 9 - Test returnArray function", function() {
  ...
  it( "Bonus Task 10 - Cast string vars to numbers", function() {
    assert.equal(castStrToNumber("5","2.2"), 7.2);
    assert.equal(castStrToNumber("7","3"), 10);
  });
  it("Bonus Task 11 - Test arrayPop function", function() {
  ...
  it('Bonus Task 12 - Test workoutTax001 function', function() {
  // test values 1
